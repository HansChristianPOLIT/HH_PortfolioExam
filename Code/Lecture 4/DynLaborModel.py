...
class DynLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        par.a_min = -10.0 # minimum point in wealth grid (allow for borrowing)
    ...
    def allocate(self):
        ...
        par.k_grid = nonlinspace(0.0,par.k_max,par.Nk,1.1) # new part relative to lecture 3
        ...
        for t in reversed(range(par.T)):
            ...
            for i_a,assets in enumerate(par.a_grid):
                for i_k,capital in enumerate(par.k_grid):
                    ...
                    if t==par.T-1: # last period (only one choice variable)
                        ...
                        # call optimizer
                        hours_min = - assets / self.wage_func(capital,t) + 1.0e-5 # minimum amout of hours that ensures positive consumption from line 112
                        hours_min = np.maximum(hours_min,2.0)
                        init_h = np.array([hours_min]) if i_a==0 else np.array([sol.h[t,i_a-1,i_k]]) # (good) initial guess on optimal hours (if not first value use last found solution)
                    ...
                    else: # here we need to take future into account as it is not the last period
                        
                        # objective function: negative since we minimize
                        obj = lambda x: - self.value_of_choice(x[0],x[1],assets,capital,t) # all we need is current state variables and choice variable and transition (assets, capital)
                        ...
                        # call optimizer
                        init = np.array([lb_c,1.0]) if (i_a==0 & i_k==0) else res.x  # initial guess on optimal consumption and hours (if not first value use last found solution)
    ...
    def value_of_choice(self,cons,hours,assets,capital,t):
        ...
        # b. penalty for violating bounds to ensure positive consumption and hours
        penalty = 0.0
        if cons < 0.0:
        ...
        V_next = sol.V[t+1] # 2d as we take out the time dimension
        income = self.wage_func(capital,t) * hours
        # state transition
        a_next = (1.0+par.r)*(assets + income - cons)
        k_next = capital + hours
        # Now we can interpolate value in next period
    ...
    def wage_func(self,capital,t):
        ...
        return (1.0 - par.tau )* par.w_vec[t] * (1.0 + par.alpha * capital) # w_vec allows us to change elasticity at different point in time
        # could have had time variation on tau but if men and women could have different wage but not tax hence better to do it on w_vec
