...
class DynHouseholdLaborModelClass(EconModelClass):
    ...
    def setup(self):
        ...
        # children
        par.p_birth = 0.1 # probability of birth
        
        # preferences
        par.beta = 0.98 # discount factor

        par.rho_1 = 0.05 # weight on labor dis-utility of men
        par.rho_1_kids = 0.05 # weight on labor dis-utility of men when children present
        par.rho_2 = 0.05 # weight on labor dis-utility of women
        par.rho_2_kids = 0.05 # weight on labor dis-utility of women when children present
    ...
    def allocate(self):
        ...
        # f. draws used to simulate child arrival and spousal presence
        np.random.seed(9210)
    ...
    def value_of_choice(self,hours1,hours2,capital1,capital2,kids,V_next):
        ...
        EV_next = 0.0
        num_birth = 1 if kids>0 else 2 # set range
        probs = [1- par.p_birth, par.p_birth]
        for birth in range(num_birth):
            ...
            EV_next += p_n_next * V_next_interp
    ...
    # relevant functions
    def consumption(self,hours1,hours2,capital1,capital2,kids): # actually the budget
        par = self.par
        ...
        # childcare costs
        child_trans = self.child_trans(hours1,hours2,income_hh,kids)
        income_hh += child_trans # added/deducted before tax here
    ...
    def child_trans(self,hours1,hours2,income_hh,kids):
        if kids <1:
            return 0.0

        else: 
            C1 = 0.01 # unconditional, universal transfer
            C2 = np.fmax(0.03 - 0.001 * income_hh, 0.0) # means-tested transfer (basetransfer and then tapered out by income), max to ensure non-negative transfer
            # childcare related (net-of-subsidy costs)
            both_work = (hours1>0) * (hours2>0)
            C3 = -0.01*both_work # all working couples has this net cost (basically 25% to have kids taken care of in DK)
            C4 = -0.01*both_work*(income_hh>0.5) # low-income couples do not have this net cost
